...
import kr.co.mostx.japi.repository.AssetScheduleRepository;
...
public class AssetSchedulerService {
    private final AssetScheduleRepository assetScheduleRepository;
    private final AssetService assetService;
    private final AssetDepreciationService assetDepreciationService;

    @Scheduled(cron = "0 0 4 * * *")
    public void run() {
        List<Asset> assets = assetScheduleRepository.findAll();
        ...
        for (Asset asset : assets) {
            if (asset.getInitialStartDate() != null && asset.getAssetStatus().equals("정상") && asset.getBookValue() != 0) {
                ...
                assetDepreciation = assetScheduleRepository.findDepreciation(asset.getSno());
